...
# Start timer
clock.schedule_interval(lambda: decrease_timer(), 0.5)

def decrease_timer():
    global timer
    if timer > 0:
        timer -= 1
        print("TIMER:", timer)  # ← Add this line
...
def draw():
    ...
    screen.draw.text(f"Timer: {int(timer)}", center=(WIDTH//2, 40), fontsize=40, color="black")
...
def update():
    global ballx, bally, score_p1, score_p2, ball_owner, ball_in_motion, prediction_timer
    ...
    if not ball_in_motion:
        if ball_owner == 'ai':
            ...
            if ai_player.x > left_hoop.centerx:
                ai_player.x -= 3
            elif ai_player.x < left_hoop.centerx:
                ai_player.x += 3
            if ai_player.y > left_hoop.centery:
                ai_player.y -= 3
            elif ai_player.y < left_hoop.centery:
                ai_player.y += 3
        else:
